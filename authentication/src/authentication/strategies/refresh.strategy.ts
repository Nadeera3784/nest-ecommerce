...
export class RefreshStrategy extends PassportStrategy(Strategy, 'refresh') {
  ...
  public async validate(payload: RefreshPayload): Promise<RefreshPayload> {
    const token: RefreshTokenInterface = await this.refreshTokenModel
      .findOne({
        _id: payload.payload.token_id,
        revoked: false,
      })
      .exec();

    if (!(token && token.token_type === TokenType.auth)) {
      throw new UnauthorizedException(
        'The token has been revoked or never existed',
      );
    }
